...
const Videos = () => {
  ...
  const [videosSrc, setVideosSrc] = useState(videosArr[0])
  return (
    <>
      <Stack direction={['column', 'row']} h='100vh'>
        <VStack w={'full'}>
          ...
          <VStack alignItems={'flex-start'} p={'8'} w={'full'} overflowY={'auto'} >
            ...
            <Text>
              This is a sample video for testing and demo.this is caled description,
            </Text>
          </VStack>
        </VStack>
        <VStack w={['full', 'xl']} alignItems={'stretch'} p={'8'} spacing={'8'} overflowY={'auto'} >
          {
            videosArr.map((items, index) => (
              <Button variant={'ghost'} colorScheme={'purple'} onClick={() => setVideosSrc(items)}>
                Lecture {index + 1}
              </Button>
            ))
          }
